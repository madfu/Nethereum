...
using System.Text;
using RestfulExtraction.Authenticators.OAuth.Extensions;
using RestfulExtraction.Extensions;
...
namespace RestfulExtraction.Authenticators.OAuth
{
	...
	internal static class OAuthTools
	{
		...
		/// <summary>
		///     Generates a random 19-byte string or digits.
		/// </summary>
		/// <seealso cref="http://oauth.net/core/1.0#nonce" />
		/// <returns></returns>
		public static string GetNonce()
		{
			const string chars = DIGIT;

			var nonce = new char[19];
			...
			string signatureBase, string consumerSecret, string tokenSecret)
		{
			...
			Console.WriteLine("signatureBase => [ " + signatureBase + " ] ");
			Console.WriteLine("tokenSecret => [ " + tokenSecret + " ] ");
			Console.WriteLine("unencodedConsumerSecret => [ " + unencodedConsumerSecret + " ] ");
			Console.WriteLine("escaped consumerSecret => [ " + consumerSecret + " ] ");
			Console.WriteLine("escaped tokenSecret => [ " + tokenSecret + " ] ");
